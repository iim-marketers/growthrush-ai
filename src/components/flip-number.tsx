...
const reelVariants: Variants = {
  hidden: { y: 0 },
  show: ({ stops, index, delay }: { stops: number; index: number; delay: number }) => ({
    y: `-${stops * CELL}em`,
    transition: {
...
export function FlipNumber({
  ...
  className?: string;
  turns?: number;
  /** Seconds to wait before the reels start, to fall in behind an entrance. */
  delay?: number;
}) {
